...
const vk = @import("dvui").backend.vulkan;
...
pub fn init(alloc: std.mem.Allocator, options: InstanceOptions) !InstanceProxy {
    ...
    const vkGetInstanceProcAddr = options.vkGetInstanceProcAddr orelse return error.VulkanNoProcAddr; //VkDLL.lookup(vk.PfnGetInstanceProcAddr, "vkGetInstanceProcAddr") orelse return error.VkHasNoGetProcAddr;
